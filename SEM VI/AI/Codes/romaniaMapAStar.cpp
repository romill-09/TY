...
void aStar(const string &start, const string &goal)
{
    ...
    while (!pq.empty())
    {
        ...
        for (auto &neighbor : graph[current])
        {
            ...
            if (gScore.find(nextCity) == gScore.end() || tentative_gScore < gScore[nextCity])
            {
                gScore[nextCity] = tentative_gScore;
                int fScore = tentative_gScore + heuristic[nextCity];
                cout << "Considering: " << nextCity << " with heuristic: " << heuristic[nextCity] << " and fScore: " << fScore << endl;
                pq.push({nextCity, fScore});
                parent[nextCity] = current;
            }
        }
        cout << endl;
    }
    cout << endl << "Goal not reachable from start." << endl;
}
